...
namespace Engine.Services
{
    ...
    {
        private const string SAVE_GAME_FILE_NAME = "JustGame.json";
        public static void Save(GameSession gameSession)
        {
            var ss = JsonConvert.SerializeObject(gameSession);
            File.WriteAllText(SAVE_GAME_FILE_NAME, ss);
        }
        /// <summary>
        /// 读取存档
        /// </summary>
        ...
        private static Player CreatePlayer(JObject data)
        {
            ...
            switch (fileVersion)
            {
                case "0.1.000":
                    player =
                        new Player((string)data[nameof(GameSession.CurrentPlayer)][nameof(Player.CharacterClass)],
        ...
        private static void PopulatePlayerInventory(JObject data, Player player)
        {
            ...
            {
                case "0.1.000":
                    foreach (JObject? itemToken in (JArray)data[nameof(GameSession.CurrentPlayer)]
        ...
        private static void PopulatePlayerQuests(JObject data, Player player)
        {
            ...
            {
                case "0.1.000":
                    foreach (JObject questToken in (JArray)data[nameof(GameSession.CurrentPlayer)]
        ...
        private static void PopulatePlayerRecipes(JObject data, Player player)
        {
            ...
            {
                case "0.1.000":
                    foreach (JObject recipeToken in
